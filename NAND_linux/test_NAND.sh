...
date
if [ $error0 -ne 0 ] ; then
  echo "NAND tests: disk-stress failed: we have an exit code of $error0"
fi
cd ../ltp-filesystems
date
...
date
if [ $error1 -ne 0 ] ; then
  echo "NAND tests: ltp-filesystems failed: we have an exit code of $error1"
fi
cd ../bonnie++-1.98
date
./bonnie++ -d /mnt -u qq
...
date
if [ $error4 -ne 0 ] ; then
  echo "NAND tests: disk-stress failed: we have an exit code of $error4"
fi
cd ../ltp-filesystems
date
./runalltests.sh -i 1024 -m 128 -p -q  -l /tmp/resultlog.11402 -d /mnt
...
date
if [ $error5 -ne 0 ] ; then
  echo "NAND tests: ltp-filesystems failed: we have an exit code of $error5"
fi
cd ../bonnie++-1.98
date
./bonnie++ -d /mnt -u qq
...
error8=$?
if [ $error8 -ne 0 ] ; then
  echo "NAND tests: disk-stress failed: we have an exit code of $error8"
fi
date
cd ../ltp-filesystems
./runalltests.sh -i 1024 -m 128 -p -q  -l /tmp/resultlog.11402 -d /mnt
error9=$?
if [ $error9 -ne 0 ] ; then
  echo "NAND tests: ltp-filesystems failed: we have an exit code of $error9"
fi
cd ../bonnie++-1.98
date
./bonnie++ -d /mnt -u qq
